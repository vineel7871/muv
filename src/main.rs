...
#[derive(Parser, Debug)]
#[clap(author, version, about = "Global environment management tool using uv", 
...
pub enum Commands {
    ...
    #[clap(long_about = "Initialize guv in your shell configuration to enable environment activation and deactivation")]
    Init(InitArgs),
    
    /// Create a new virtual environment
    #[clap(long_about = "Create a new Python virtual environment with the specified name and Python version")]
    Create(CreateArgs),
    ...
    #[clap(long_about = "Activate a virtual environment to use its Python interpreter and packages")]
    Activate(EnvNameArg),
    ...
    #[clap(long_about = "Generate a requirements.txt-compatible list of all installed packages in the environment")]
    Freeze(EnvNameArg),
...
pub struct CreateArgs {
    ...
    #[clap(help = "Name of the environment to create", long_help = "Specify a name for the new virtual environment. This name will be used to reference the environment in other commands.")]
    pub name: String,
    
    /// Python version to use (e.g., 3.10, python3.11, /usr/bin/python3)
    #[clap(short, long, help = "Python version to use", long_help = "Specify which Python version to use for this environment. Can be a version number (e.g., 3.10), an interpreter name (e.g., python3.11), or a path to a Python executable.")]
...
pub struct InitArgs {
    ...
    #[clap(long, help = "Force re-initialization", long_help = "Force re-initialization of guv in your shell configuration even if it's already set up. This will replace any existing guv configuration.")]
    pub force: bool,
}
...
pub struct EnvNameArg {
    ...
    #[clap(value_name = "ENV_NAME", help = "Environment name", long_help = "Specify the name of the environment to use. If not provided and an environment is currently active, the active environment will be used.")]
...
pub struct DeleteArgs {
    ...
    #[clap(help = "Environment to delete", long_help = "Specify the name of the environment you want to permanently delete.")]
    pub name: String,
    
    /// Skip confirmation prompt
    #[clap(short, long, help = "Skip confirmation", long_help = "Skip the confirmation prompt and immediately delete the environment. Use with caution as this action cannot be undone.")]
    pub yes: bool,
}
...
pub struct PackageManagementArgs {
    /// Name of the environment (optional if an environment is active)
    #[clap(short, long, value_name = "ENV_NAME", help = "Environment name", long_help = "Specify the name of the environment to manage packages in. If not provided and an environment is currently active, the active environment will be used.")]
...
pub struct RunArgs {
    ...
    #[clap(help = "Environment name", long_help = "Specify the name of the environment in which to run the command.")]
    pub env_name: String,
